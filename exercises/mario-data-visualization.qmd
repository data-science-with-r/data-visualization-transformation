...
In this demonstration, we are going to explore if there is a difference in mean total price by if the game was sold with at least 

### 

### `filter()`

The function `filter()` acts on the rows of the data set, and subsets the data set based on a condition. Let's use the `filter()` function 
