...
We'll use: **tidyverse** for majority of the analysis and **scales** for pretty plot labels later on. **ggridges** allows us to make ridge plots, and **gridExtra** allows to arrange plots next to each other. These are ready to use for you in this activity!
...
mario |>
  ggplot(
    aes(x = cond , fill = ship_sp,  position = #add code here, #add code here)) + 
   geom_bar()
```
